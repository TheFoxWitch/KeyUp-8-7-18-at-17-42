 ...
 */
$title = $source = $image = $custom_src = $onclick = $img_size = $external_img_size = $video_link =
$caption = $img_link_large = $link = $img_link_target = $alignment = $el_class = $el_id = $css_animation = $style = $external_style = $border_color = $css = $overflow = $max_width = $max_height = $overflow_pos = $hide_lg = $hide_md = $hide_sm = '';
...
switch ( $onclick ) {
	...
	case 'cesis_lightbox':
		...
		$a_attrs['class'] = 'cesis_lightbox_el';
		if ( 'external_link' === $source ) {
			$link = $custom_src;
		} else {
			$link = wp_get_attachment_image_src( $img_id, 'large' );
...
if($overflow == 'yes'){
	...
	$overflow_style = ' style="width:'.$max_width.'px; max-width:none;" ';
	$wrapper_attributes[] = ' style="min-height:' . esc_attr( $max_height ) . 'px; position:relative;" ';
}else {
	$overflow_pos = '';
	$overflow_style = '';
}
...
if ( $link ) {
	...
	$html = '<a ' . vc_stringify_attributes( $a_attrs ) . ' data-src="'.$link.'" '.' class="' . $wrapperClass . '" '.$overflow_style.'>' . $img['thumbnail'] . '</a>';
} else {
	$html = '<div class="' . $wrapperClass . '"  '.$overflow_style.'>' . $img['thumbnail'] . '</div>';
}


$class_to_filter = 'wpb_single_image wpb_content_element vc_align_' . $alignment . ' ' . $overflow_pos . ' ' . $hide_lg  .' '. $hide_md .' '. $hide_sm .' '. $this->getCSSAnimation( $css_animation );
