 ...
 */
$source = $text = $link = $google_fonts = $font_container = $el_id = $el_class = $css = $css_animation = $font_container_data = $google_fonts_data = $text_transform = $letter_spacing = $delay = $text_resize = $text_minimum = array();
 ...
 * @var $css_class
 */
extract( $this->getStyles( $el_class .' '. $text_transform .' '. $hide_lg  .' '. $hide_md .' '. $hide_sm .' '. $this->getCSSAnimation( $css_animation ), $css, $google_fonts_data, $font_container_data, $atts ) );
...
if ( $text_resize == 'yes' ) {
	$wrapper_attributes[] = 'data-max_size="' . esc_attr( $font_container_data['values']['font_size'] ) . '" data-min_size="' . esc_attr( $text_minimum ) . '" ';
